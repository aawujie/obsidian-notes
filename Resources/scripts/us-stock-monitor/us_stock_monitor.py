...
def generate_report(df, date_str):
    ...
    lines += [
        ...
        f"| 持平 | {flat} |",
        f"| 平均日涨幅 | {safe_pct(avg_daily)} |",
        f"| 平均周涨幅 | {safe_pct(avg_weekly)} |",
        f"| 平均月涨幅 | {safe_pct(avg_monthly)} |",
    ...
    if df["sector"].notna().any() and (df["sector"] != "").any():
        ...
        for sector, srow in sector_stats.iterrows():
            if sector == "":
                continue
            lines.append(
                f"| {sector} | {safe_pct(srow['日涨幅'])} | "
                f"{safe_pct(srow['周涨幅'])} | {safe_pct(srow['月涨幅'])} | "
