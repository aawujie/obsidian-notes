...
> 面试官你好，我叫吴杰，做了 5 年 C++ 和 Python 开发，一直在自动驾驶方向。
>
> 前三年在禾多科技，纯 C++，主要做底层系统。最开始写传感器驱动——激光雷达、相机、IMU、GNSS 这些，不同厂商的硬件做了统一抽象，上层算法不用感知差异。多传感器的时间同步也做过，GNSS 做主时钟，硬件触发对齐时间戳。后来独立负责了 IFC 域控的 Autosar 迁移，把整套算法从 ARM 平台搬到 x86，让开发人员一台 PC 就能完成全部调试，不用依赖实车和域控硬件。
>
