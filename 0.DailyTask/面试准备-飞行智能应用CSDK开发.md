...
> 面试官您好，我叫吴杰，有 5 年 C++ 和 Python 开发经验，一直在自动驾驶方向。
>
> 前三年在禾多科技，纯 C++开发。最开始写传感器驱动——激光雷达、相机、IMU这些，把不同厂商的硬件用同一套接口封起来用同一消息格式发送，目的是让上层算法不用管底层是谁家的设备。传感器相关的项目做过数采车上的多传感器的时间同步，GNSS 做主时钟，硬件触发对齐各传感器时间戳。后来独立负责 SIL 项目，负责采集地平线感知数据，把前视智能一体机域控的 Autosar 平台迁移，把整套嵌入式系统从 ARM 平台搬到 x86，让测试人员一台 PC 就能完成全部调试，不用依赖实车和域控硬件。
>
> 现在在元戎启行，负责 HIL 自动化测试平台，从 pytest 测试架构设计到 gitlab CI/CD 再到 grafana 监控看板都是独立主导的，目前每天跑 36 万条自动化用例、覆盖 14 个业务域。
> 
> 工作之余我也自己会看一些算法方面的知识。
> 
> 我看到这个岗位的时候觉得特别合适——自动驾驶和无人机在底层技术上其实是通的，传感器、通信协议、实时系统这些我都做过，我的经验可以直接迁移过来。
