...
│ │  成员变量1    │ │   ┌───────────┐  │ │  (Base成员)   │    │    ┌─────────────────────┐
│ │  成员变量2    │ │   │ Base::f1  │  │ │  成员变量1    │    │    │ &Derived::func1()    │ ← 覆盖了
│ └──────────────┘ │   │ Base::f2  │  │ │  成员变量2    │    │    │ &Base::func2()       │ ← 没覆盖
└─────────────────┘   │ Base::~B  │  │ └──────────────┘    │    │ &Derived::~Derived() │ ← 覆盖了
                      └───────────┘  └─────────────────────┘    └─────────────────────┘
```
...
| 项 | 作用 | 调大效果 | 过大问题 |
|----|------|---------|---------|
| P | 快速响应误差 | 响应快、稳态误差小 | 超调、振荡 |
| I | 消除稳态误差（静差） | 消除静差 | 积分饱和、超调、响应变慢 |
| D | 预测趋势、抑制超调 | 阻尼增大、减小超调 | 放大高频噪声、系统不稳定 |

**调参顺序：** 先 P 调到有轻微超调 → 加 D 抑制超调 → 最后加 I 消除静差。I 最危险，最容易导致不稳定。

**积分饱和（Integral Windup）：** 当执行器饱和（如电机到最大转速）时，误差持续存在，积分项不断累积到极大值，导致系统失控。解法：积分限幅、积分分离（误差大时不用 I）、反计算抗饱和。
