...
> 面试官你好，我叫吴杰，有 5 年 C++ 和 Python 开发经验，一直在自动驾驶方向。
>
> 前三年在禾多科技，纯 C++，主要做底层系统。最开始写传感器驱动——激光雷达、相机、IMU这些，不同厂商的硬件用同一套接口封起来，上层算法不用管底层是谁家的设备。多传感器的时间同步也做过，GNSS 做主时钟，硬件触发对齐时间戳。后来独立负责了 IFC 域控的 Autosar 迁移，把整套嵌入式系统从 ARM 平台搬到 x86，让开发人员一台 PC 就能完成全部调试，不用依赖实车和域控硬件。
>
> 现在在元戎启行，负责 HIL 自动化测试平台，从架构设计到 CI/CD 到监控看板都是独立主导的，目前每天跑 36 万条自动化用例、覆盖 14 个业务域。
> 
> 工作之余我也自己看一些算法方面的
> 
> 我看到这个岗位的时候觉得特别合适——自动驾驶和无人机在底层技术上其实是通的，传感器、通信协议、实时系统这些我都做过，我相信我的经验可以直接迁移过来。
