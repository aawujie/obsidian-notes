...
| **Bash** | 运行一系列 shell 命令 | 命令输出冗长，隔离后父代理专注决策 |
| **Browser** | 通过 MCP 工具控制浏览器 | 过滤噪声 DOM 快照和截图，返回相关结果 |
...
**解决方案**：
- 上下文隔离 → 父代理只接收最终摘要
- 模型灵活性 → explore 默认使用更快模型
- 专用配置 → 针对特定任务优化
- 成本效率 → 高 token 开销工作隔离到合适模型
...
|---------------------|---------------------|
| 需要为长时间研究任务进行上下文隔离 | 任务是单一用途的（生成 changelog、格式化等） |
