...
# 12.1\. 编译器和解释器[¶](#sec-hybridize "Permalink to this heading")

[  Colab \[mxnet\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-colab/blob/master/chapter%5Fcomputational-performance/hybridize.ipynb)

 Open the notebook in Colab

[  Colab \[pytorch\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-pytorch-colab/blob/master/chapter%5Fcomputational-performance/hybridize.ipynb)

 Open the notebook in Colab

[  Colab \[tensorflow\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-tensorflow-colab/blob/master/chapter%5Fcomputational-performance/hybridize.ipynb)

 Open the notebook in Colab

[  Colab \[paddle\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-paddle-colab/blob/master/chapter%5Fcomputational-performance/hybridize.ipynb)
...
目前为止，本书主要关注的是_命令式编程_（imperative programming）。 命令式编程使用诸如`print`、“`+`”和`if`之类的语句来更改程序的状态。 考虑下面这段简单的命令式程序：
...
Python是一种_解释型语言_（interpreted language）。因此，当对上面的`fancy_func`函数求值时，它按顺序执行函数体的操作。也就是说，它将通过对`e = add(a, b)`求值，并将结果存储为变量`e`，从而更改程序的状态。接下来的两个语句`f = add(c, d)`和`g = add(e, f)`也将执行类似地操作，即执行加法计算并将结果存储为变量。[图12.1.1](#fig-compute-graph)说明了数据流。

图12.1.1 命令式编程中的数据流[¶](#id8 "Permalink to this image")
...
## 12.1.1\. 符号式编程[¶](#id2 "Permalink to this heading")

考虑另一种选择_符号式编程_（symbolic programming），即代码通常只在完全定义了过程之后才执行计算。这个策略被多个深度学习框架使用，包括Theano和TensorFlow（后者已经获得了命令式编程的扩展）。一般包括以下步骤：
...
## 12.1.2\. 混合式编程[¶](#id3 "Permalink to this heading")
...
命令式编程现在是TensorFlow2的默认选择，对那些刚接触该语言的人来说是一个很好的改变。不过，符号式编程技术和计算图仍然存在于TensorFlow中，并且可以通过易于使用的装饰器`tf.function`进行访问。这为TensorFlow带来了命令式编程范式，允许用户定义更加直观的函数，然后使用被TensorFlow团队称为[autograph](https://www.tensorflow.org/api%5Fdocs/python/tf/autograph)的特性将它们封装，再自动编译成计算图。
...
## 12.1.3\. `Sequential`的混合式编程[¶](#sequential "Permalink to this heading")
...
### 12.1.3.1\. 通过混合式编程加速[¶](#id4 "Permalink to this heading")
...
### 12.1.3.2\. 序列化[¶](#id5 "Permalink to this heading")
...
## 12.1.4\. 小结[¶](#id6 "Permalink to this heading")
...
## 12.1.5\. 练习[¶](#id7 "Permalink to this heading")
...
[](https://discuss.d2l.ai/t/2787)

[](https://discuss.d2l.ai/t/11857)
...
* [12.1\. 编译器和解释器](#)  
   * [12.1.1\. 符号式编程](#id2)  
   * [12.1.2\. 混合式编程](#id3)  
   * [12.1.3\. Sequential的混合式编程](#sequential)  
         * [12.1.3.1\. 通过混合式编程加速](#id4)  
         * [12.1.3.2\. 序列化](#id5)  
   * [12.1.4\. 小结](#id6)  
   * [12.1.5\. 练习](#id7)

[ Previous 12\. 计算性能 ](index.html) [ Next 12.2\. 异步计算 ](async-computation.html)