...
# 13.2\. 微调[¶](#sec-fine-tuning "Permalink to this heading")

[  Colab \[mxnet\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-colab/blob/master/chapter%5Fcomputer-vision/fine-tuning.ipynb)

 Open the notebook in Colab

[  Colab \[pytorch\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-pytorch-colab/blob/master/chapter%5Fcomputer-vision/fine-tuning.ipynb)

 Open the notebook in Colab

[  Colab \[tensorflow\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-tensorflow-colab/blob/master/chapter%5Fcomputer-vision/fine-tuning.ipynb)

 Open the notebook in Colab

[  Colab \[paddle\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-paddle-colab/blob/master/chapter%5Fcomputer-vision/fine-tuning.ipynb)
...
另一种解决方案是应用_迁移学习_（transfer learning）将从_源数据集_学到的知识迁移到_目标数据集_。 例如，尽管ImageNet数据集中的大多数图像与椅子无关，但在此数据集上训练的模型可能会提取更通用的图像特征，这有助于识别边缘、纹理、形状和对象组合。 这些类似的特征也可能有效地识别椅子。

## 13.2.1\. 步骤[¶](#id2 "Permalink to this heading")

本节将介绍迁移学习中的常见技巧:_微调_（fine-tuning）。如[图13.2.1](#fig-finetune)所示，微调包括以下四个步骤。
...
图13.2.1 微调。[¶](#id9 "Permalink to this image")

当目标数据集比源数据集小得多时，微调有助于提高模型的泛化能力。

## 13.2.2\. 热狗识别[¶](#id3 "Permalink to this heading")
...
### 13.2.2.1\. 获取数据集[¶](#id4 "Permalink to this heading")
...
[07:18:14] ../src/storage/storage.cc:196: Using Pooled (Naive) StorageManager for CPU
...
在训练期间，我们首先从图像中裁切随机大小和随机长宽比的区域，然后将该区域缩放为\\(224 \\times 224\\)输入图像。 在测试过程中，我们将图像的高度和宽度都缩放到256像素，然后裁剪中央\\(224 \\times 224\\)区域作为输入。 此外，对于RGB（红、绿和蓝）颜色通道，我们分别_标准化_每个通道。 具体而言，该通道的每个值减去该通道的平均值，然后将结果除以该通道的标准差。
...
### 13.2.2.2\. 定义和初始化模型[¶](#id5 "Permalink to this heading")
...
Downloading /opt/mxnet/models/resnet18_v2-a81db45f.zip10bf5ec0-9aba-425f-83bf-d8b33fad128b from https://apache-mxnet.s3-accelerate.dualstack.amazonaws.com/gluon/models/resnet18_v2-a81db45f.zip...
...
W0818 09:25:36.300442 94354 gpu_resources.cc:61] Please NOTE: device: 0, GPU Compute Capability: 7.0, Driver API Version: 11.8, Runtime API Version: 11.8
W0818 09:25:36.341431 94354 gpu_resources.cc:91] device: 0, cuDNN Version: 8.7.

预训练的源模型实例包含许多特征层和一个输出层`fc`。 此划分的主要目的是促进对除输出层以外所有层的模型参数进行微调。 下面给出了源模型的成员变量`fc`。

pretrained_net.fc

Linear(in_features=512, out_features=1000, dtype=float32)
...
### 13.2.2.3\. 微调模型[¶](#id6 "Permalink to this heading")
...
我们使用较小的学习率，通过_微调_预训练获得的模型参数。

[mxnet](#mxnet-17-0)[pytorch](#pytorch-17-1)[paddle](#paddle-17-2)

train_fine_tuning(finetune_net, 0.01)

loss 0.289, train acc 0.919, test acc 0.931
379.9 examples/sec on [gpu(0), gpu(1)]
...
loss 0.193, train acc 0.920, test acc 0.921
915.7 examples/sec on [Place(gpu:0), Place(gpu:1)]
...
loss 0.359, train acc 0.848, test acc 0.845
323.7 examples/sec on [gpu(0), gpu(1)]
...
loss 0.348, train acc 0.849, test acc 0.848
1074.7 examples/sec on [Place(gpu:0), Place(gpu:1)]

意料之中，微调模型往往表现更好，因为它的初始参数值更有效。

## 13.2.3\. 小结[¶](#id7 "Permalink to this heading")

* 迁移学习将从源数据集中学到的知识_迁移_到目标数据集，微调是迁移学习的常见技巧。
* 除输出层外，目标模型从源模型中复制所有模型设计及其参数，并根据目标数据集对这些参数进行微调。但是，目标模型的输出层需要从头开始训练。
* 通常，微调参数使用较小的学习率，而从头开始训练输出层可以使用更大的学习率。

## 13.2.4\. 练习[¶](#id8 "Permalink to this heading")
...
1. 事实上，`ImageNet`数据集中有一个“热狗”类别。我们可以通过以下代码获取其输出层中的相应权重参数，但是我们怎样才能利用这个权重参数？
...
* [13.2\. 微调](#)  
   ...
   * [13.2.2\. 热狗识别](#id3)  
         * [13.2.2.1\. 获取数据集](#id4)  
         * [13.2.2.2\. 定义和初始化模型](#id5)  
         * [13.2.2.3\. 微调模型](#id6)  
   * [13.2.3\. 小结](#id7)  
   * [13.2.4\. 练习](#id8)

[ Previous 13.1\. 图像增广 ](image-augmentation.html) [ Next 13.3\. 目标检测和边界框 ](bounding-box.html)