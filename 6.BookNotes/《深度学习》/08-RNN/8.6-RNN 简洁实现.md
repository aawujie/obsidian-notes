---
title: 8.6-RNN 简洁实现
source: https://zh.d2l.ai/chapter_recurrent-neural-networks/rnn-concise.html
converted_at: 2026-02-28
---

# 8.6\. 循环神经网络的简洁实现[¶](#sec-rnn-concise "Permalink to this heading")

[  Colab \[mxnet\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-colab/blob/master/chapter%5Frecurrent-neural-networks/rnn-concise.ipynb)

 Open the notebook in Colab

[  Colab \[pytorch\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-pytorch-colab/blob/master/chapter%5Frecurrent-neural-networks/rnn-concise.ipynb)

 Open the notebook in Colab

[  Colab \[tensorflow\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-tensorflow-colab/blob/master/chapter%5Frecurrent-neural-networks/rnn-concise.ipynb)

 Open the notebook in Colab

[  Colab \[paddle\] ](https://colab.research.google.com/github/d2l-ai/d2l-zh-paddle-colab/blob/master/chapter%5Frecurrent-neural-networks/rnn-concise.ipynb)
...
虽然 [8.5节](rnn-scratch.html#sec-rnn-scratch)对了解循环神经网络的实现方式具有指导意义，但并不方便。 本节将展示如何使用深度学习框架的高级API提供的函数更有效地实现相同的语言模型。 我们仍然从读取时光机器数据集开始。
...
## 8.6.1\. 定义模型[¶](#id2 "Permalink to this heading")

高级API提供了循环神经网络的实现。 我们构造一个具有256个隐藏单元的单隐藏层的循环神经网络层`rnn_layer`。 事实上，我们还没有讨论多层循环神经网络的意义（这将在[9.3节](../chapter%5Frecurrent-modern/deep-rnn.html#sec-deep-rnn)中介绍）。 现在仅需要将多层理解为一层循环神经网络的输出被用作下一层循环神经网络的输入就足够了。
...
与 [8.5节](rnn-scratch.html#sec-rnn-scratch)类似， 我们为一个完整的循环神经网络模型定义了一个`RNNModel`类。 注意，`rnn_layer`只包含隐藏的循环层，我们还需要创建一个单独的输出层。
...
## 8.6.2\. 训练与预测[¶](#id3 "Permalink to this heading")
...
很明显，这种模型根本不能输出好的结果。 接下来，我们使用[8.5节](rnn-scratch.html#sec-rnn-scratch)中 定义的超参数调用`train_ch8`，并且使用高级API训练模型。
...
## 8.6.3\. 小结[¶](#id4 "Permalink to this heading")

* 深度学习框架的高级API提供了循环神经网络层的实现。
* 高级API的循环神经网络层返回一个输出和一个更新后的隐状态，我们还需要计算整个模型的输出层。
* 相比从零开始实现的循环神经网络，使用高级API实现可以加速训练。

## 8.6.4\. 练习[¶](#id5 "Permalink to this heading")

1. 尝试使用高级API，能使循环神经网络模型过拟合吗？
2. 如果在循环神经网络模型中增加隐藏层的数量会发生什么？能使模型正常工作吗？
3. 尝试使用循环神经网络实现 [8.1节](sequence.html#sec-sequence)的自回归模型。
...
[](https://discuss.d2l.ai/t/5766)

[](https://discuss.d2l.ai/t/11800)
...
[ Previous 8.5\. 循环神经网络的从零开始实现 ](rnn-scratch.html) [ Next 8.7\. 通过时间反向传播 ](bptt.html)