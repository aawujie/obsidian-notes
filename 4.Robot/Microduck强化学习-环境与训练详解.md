...
**域随机化(Domain Randomization,DR)= 训练时不给策略看"一个固定仿真",而是每个 episode 随机打乱物理参数——让策略学会在"一大堆略有不同的世界里都表现好"。** 概念出自 Tobin et al. 2017,核心论证:**"如果仿真变化范围足够大,真实世界就是这堆变化里的一个样本。"** 与其把"一个仿真"调到和现实一致(做不到),不如做成一个分布,让真实落在分布内部——策略见到真机就是"又一个随机变体"。

| 字段 | 范围 | 模拟什么 |
|---|---|---|
| 躯干 CoM | ±3mm(课程到 ±8) | 重心偏差 |
| 头部 CoM | ±3mm | 头组装误差 |
| 质量/惯量 | ±5% | 打印/电池重量差异 |
| 关节摩擦 | ±10% | 舵机个体差异 |
| armature | ±10% | 反射惯量误差 |
| **速度推搡** | 每 3-6s ±0.3 m/s(原 ±0.5 审计后收紧) | 外力扰动,训练抗摔 |
| IMU 安装误差 | ≤6° 随机轴 | 传感器贴歪 |
| 编码器偏置 | ±0.86° | 角度零位误差 |
| 初始倾斜 | 俯仰 ±10°/横滚 ±5° | 从歪的姿势起步 |
| 电池电压 | 6.5–8.2V | 电量变化 |
| 电压跌落 / 命令延迟 | 随机 | BAM 执行器内部 |
| 地形 | flat / rough | 地面粗糙度 |

**应用细节**(决定成败):
- **每环境一个样本**:4096 环境重置时各自采样一组参数,一轮里同时看到轻鸭/重鸭/歪 IMU/旧电池
- **不累积**:每次重置重新采样,误差不能跨 episode 叠加(审计发现累积式 DR 越滚越离谱)
- **幅度要克制**:±0.5 推搡收成 ±0.3——注释原话:"踢得比最大步速还大,训练出的是一只永远紧张兮兮的鸭子"

**和课程学习 / BAM 的关系**:课程调难度、DR 调扰动、BAM 调物理保真度——DR 补误差,物理保真补底子,三者叠加才是完整 sim2real 配方(microduck_rl 原话:执行器保真度占 sim2real 差距的大头)。
