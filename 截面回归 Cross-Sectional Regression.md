...
## 1. 核心定位：解释“为什么不同？”
*   **视角切换**：
    *   **时序回归**关注：为什么**同一个资产**今天涨明天跌？（时间变化）
    *   **截面回归**关注：为什么**这个资产**比**那个资产**赚得多？（横向差异）
*   **核心优势**：
    它不需要因子本身是“可交易的投资组合”。即使因子是“GDP 增长率”这种宏观数据，只要能算出资产对它的敏感度（$\beta$），截面回归就能算出这个因子值多少钱（溢价 $\lambda$）。

---

## 2. 标准操作：两步走 (Two-Pass)

截面回归通常不能一步到位，必须分两步（这也是它被称为 **Two-Pass Regression** 的原因）：

### 第一步：估算 $\beta$ (时间序列)
*   **动作**：先看看每个资产对因子的敏感程度。
*   **方法**：用历史数据做时序回归。
    *   $$ R_{it} = a_i + \beta_i f_t + \epsilon_{it} $$
*   **产出**：得到每个资产的因子暴露 **$\hat{\beta}_i$**。

### 第二步：估算 $\lambda$ (横截面)
*   **动作**：拿着第一步算出来的 $\beta$ 当作“自变量”，去解释资产的平均收益。
*   **公式 (2.18)**：
    $$ E[R_i^e] = \gamma + \hat{\beta}_i \lambda + \alpha_i $$
*   **目标**：
    *   **最小化**所有资产定价误差 ($\alpha_i$) 的平方和。
    *   **求解**因子风险溢价 ($\lambda$)。
...
*   **截面回归（柔性）**：
    *   它认为：因子的历史平均收益可能有噪声，不准确。我宁愿根据资产的表现倒推因子值多少钱。
...
1.  **OLS (普通最小二乘)**：
    ...
    *   **缺点**：容易受波动大的小盘股干扰，也忽略了行业板块内的联动。
2.  **GLS (广义最小二乘)**：
    *   引入资产协方差矩阵 $\boldsymbol{\Sigma}$。
    *   **优点**：给波动小、独立的资产更高权重。在统计上是最有效（Efficient）的估计。
