...
| 术语     | 位置       | 特征类型                  | 梯度更新               |
| ------ | -------- | --------------------- | ------------------ |
| **下层/底层** | 靠近输入    | 底层特征（边缘、纹理、角点）       | 更新慢（梯度消失）         |
| **上层/高层** | 靠近输出    | 高层特征（语义、概念、物体）       | 更新快（梯度先到）         |
...
输入图像
   ↓
Conv1_1, Conv1_2  ← 下层（提取边缘、纹理）
   ↓
Conv2_1, Conv2_2  ← 中层
   ↓
...
   ↓
Conv5_1, Conv5_2  ← 上层（提取语义概念）
   ↓
全连接层          ← 最高层（分类决策）
   ↓
输出
