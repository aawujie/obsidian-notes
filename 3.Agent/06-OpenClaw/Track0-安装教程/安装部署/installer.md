...
> [!info] 设计用于希望将所有内容放在本地前缀（默认 `~/.openclaw`）下且不依赖系统 Node 的环境。
> :::
>
>
> ### 流程（install-cli.sh）
...
> [!abstract]- Windows："npm error spawn git / ENOENT"
> 安装 Git for Windows，重新打开 PowerShell，重新运行安装器。


> [!abstract]- Windows："openclaw is not recognized"
> 运行 `npm config get prefix`，追加 `\bin`，将该目录添加到用户 PATH，然后重新打开 PowerShell。


> [!abstract]- Windows：如何获取详细安装器输出
