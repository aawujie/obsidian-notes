...
当你使用大量第三方库和 harness，你把自己锁死在了一个"解决方案"里——而这个解决方案解决的问题，可能在下一代模型里根本不存在。

### 最重要的事实

> 前沿公司员工是 agent 的最大用户，无限 token 预算，最新模型。如果真有一个问题存在且有好的解决方案，前沿公司会是那解决方案的最大用户。然后他们会把那解决方案**直接整合进产品**。

看看 skills、memory harness、subagents——它们都始于"对真实问题的解决方案"，被验证有效后，被整合进了基础产品。

> 所以，如果某个东西真的是突破性的，它迟早会被整合进基础产品。**你不需要安装任何外部依赖来做最好的工作。**
...
**你要给 agent 恰好完成任务所需的信息，仅此而已。**

你对上下文的控制越好，agent 表现越好。一旦引入花里胡哨的记忆系统、插件、命名糟糕的 skills，你就是在让 agent 同时学习造炸弹和烤蛋糕，而你想要它写的只是一首关于红杉林的小诗。
...
**模糊指令：** "去建一个认证系统"
→ agent 需要研究什么是认证系统？有哪些选项？优劣？搜遍网络，上下文被各种实现细节填满。到真正实现时，已经混淆了，开始幻觉。

**精准指令：** "实现 JWT 认证，bcrypt-12 密码哈希，refresh token 轮换，7天过期"
→ agent 不需要研究其他方案，上下文全是实现细节。

### 如果你不知道实现细节？

1. 创建一个**研究任务**，让 agent 研究各种实现方案
2. 你自己决定（或让 agent 决定）用哪个方案
3. 用一个**全新上下文**的 agent 去实现
...
> 没人会用一款天天骂你、告诉你你错了、完全无视你指令的产品。所以这些 agent 被设计成尽可能同意你、做你想让它做的事。

### 问题

你说"帮我在代码库里找个 bug"——它一定会找到 bug，**哪怕它需要自己造一个**。因为它极度想遵循你的指令。

大多数人抱怨 LLM 幻觉，却没意识到**自己才是问题**。你问什么，它就会给什么——哪怕需要稍微扭曲一下事实。
...
中性 prompt 有时会发现 bug，有时只是如实陈述代码如何运行。但它不会把 agent 偏向"一定存在 bug"的方向。
...
| Agent | 角色 | 指令 | 产出 |
|:---|:---|:---|:---|
| Bug-Finder | 找 bug | +1低影响/+5中等/+10严重 | 所有可能 bug 的超集（包括误报） |
...
> 如果 OpenAI 和 Claude 都实现了它，或者收购了实现它的公司——那它大概率有用。
...
> 你不需要"保持更新"。**只需要偶尔更新你的 CLI 工具，读一下新增了什么功能。** 这就够了。
