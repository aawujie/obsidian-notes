...
agent 有时像世界上最聪明的东西，有时又蠢得令人发指。区别在于：**agent 是否被迫做了假设或"填补空白"**。

> 截至目前，agent 在"连接点"、"填补空白"、"做假设"方面仍然糟糕透顶。一旦它们开始假设，立刻就能看出它们走上了歧途。

**最重要的 CLAUDE.md 规则：**
- 告诉 agent 在 compaction 后重读任务计划
- 重读与任务相关的文件
- 在继续之前重新建立上下文
...
人类对"任务完成"有很强的直觉。agent 最大的问题是：**知道怎么开始，不知道怎么结束。** 这导致 agent 实现一堆 stub 然后交差。

### 解决方案：测试 + 合约

> 测试是 agent 的极好里程碑——它们是确定性的，你可以设定清晰的期望。

**"{TASK}_CONTRACT.md"**：规定任务可以结束之前必须完成什么。
- 测试必须全部通过
- 不允许修改测试
- 截图 + 验证（设计或行为）
...
> 24 小时连续运行的 agent 并不可取——它强制引入了上下文膨胀，无关的合同内容会污染 session。
