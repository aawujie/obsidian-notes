...
**把 Agent 想象成一个聪明但上下文信息有限的人类开发者。** 要让它正确完成任务，最好的方式是提供足够的上下文，让它明白该做什么。
...
正确的环境配置 = 更多的隐式上下文。Agent 不需要猜测依赖、命令或配置。

### 2. 工具设计决定能力上限

Agent 只能做它的工具允许它做的事。投资于工具开发 = 投资于 Agent 能力。

### 3. 迭代是必须的

第一次创建的工具很少是完美的。观察 Agent 如何使用，然后改进。

### 4. 减少噪声

Agent 容易被过多信息分散注意力。过滤日志、简化输出、聚焦关键信息。
